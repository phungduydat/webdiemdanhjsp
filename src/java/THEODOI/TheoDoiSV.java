 ...
 * @author duyda
 */
@WebServlet(name = "TheoDoiSV", urlPatterns = {"/tablesv"})
public class TheoDoiSV extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (classno == null) {
                ...
                request.getRequestDispatcher("selcet_class_2.jsp").forward(request, response);
            } else {
                 Classmenberandsch d = dao.gettl("cnpm");
            request.setAttribute("classno", d);
                HttpSession session = request.getSession();
                session.setAttribute("classsno", classno);
                ...
                request.getRequestDispatcher("TheoDoiSV.jsp").forward(request, response);
            }
